...
func (d *RelaysDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_relays"
}

func (d *RelaysDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (d *RelaysDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *RelaysDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	items, err := d.client.ListRelays(ctx)
	if err != nil {
		resp.Diagnostics.AddError("Error Reading Relays", err.Error())
		return
	}

	for _, i := range items {
		state.Items = append(state.Items, RelayModel{
