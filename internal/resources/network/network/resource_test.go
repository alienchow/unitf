...
func TestAccNetworkResource_SchemaValidation(t *testing.T) {
	resource.UnitTest(t, resource.TestCase{
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			{
				Config:             getTestData(t, "network_valid.tf"),
				ExpectError:        nil,
				PlanOnly:           true,
				ExpectNonEmptyPlan: true,
			},
			{
				Config:             getTestData(t, "network_invalid.tf"),
				ExpectError:        regexp.MustCompile(`Invalid Attribute Value Match`), // Schema validator will catch it
